...
import { getBannersInternos, getBannersInternosCategory } from 'api/bannerInternosAPI';
...
export default function BannerInterno({ title, img, description }) {
  ...
  let backgroundImage = dataBanner.Imagen_de_fondo.data[0] ? dataBanner?.Imagen_de_fondo?.data[0]?.attributes?.url : dataBanner?.Imagen_de_fondo?.data?.attributes?.url;
  return dataBanner ? (
    <div
    ...
    >
      <div className="content-image">
        <div className="!w-full center-data  md:!w-3/4 lg:!w-1/2">
          ...
          <p className="mt-3 text-center font-semibold text-sm xl:text-base text-white !leading-5">{description ? description : dataBanner?.Descripcion}</p>
        </div>
      </div>
    </div>
  ) : (
