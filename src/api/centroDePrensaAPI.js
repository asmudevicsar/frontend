...
export async function getEventsComunicadosGallery() {
  try {
    const url = `${API_URL}/api/eventos-y-noticias?sort[0]=id:desc&populate=*`;
    const response = await fetch(url);
    const result = await response.json();

    const url2 = `${API_URL}/api/comunicados-de-prensas?sort[0]=id:desc&populate=*`;
    const response2 = await fetch(url2);
    const result2 = await response2.json();

    const arr3 = result.data.concat(result2.data);
    console.log("aaaannn",arr3)
    return arr3;
  } catch (error) {
    console.log(error);
    return null;
...
export async function getNoticesDataMaxSix() {
  try {
    const url = `${API_URL}/api/eventos-y-noticias?pagination[limit]=6&sort[0]=id:desc&populate=*`;
    const response = await fetch(url);
    const result = await response.json();
...
export async function getComunicadosPrensaData() {
  try {
    const url = `${API_URL}/api/comunicados-de-prensas?sort[0]=id:desc&populate=*`;
    const response = await fetch(url);
    const result = await response.json();
...
export async function getComunicadosPrensaDataMaxSix() {
  try {
    const url = `${API_URL}/api/comunicados-de-prensas?pagination[limit]=6&sort[0]=id:desc&populate=*`;
    const response = await fetch(url);
    const result = await response.json();
