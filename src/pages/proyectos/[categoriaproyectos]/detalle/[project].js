...
export default function ProjectDetail() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const responseCategoryData = await getProjectsAll(
        response?.data[0]?.attributes?.Categoria_del_proyecto?.data?.id
      );
      responseCategoryData?.data?.map((itemcategory) =>
        arrIdProjects.push(itemcategory?.id)
      );

      if (arrIdProjects.length > 0) {
        positionCurrently = arrIdProjects.findIndex(
          (item) => item == query.query.project
        );
  ...
  return (
    <>
      <Seo
        ...
        description={data?.attributes?.Titulo}
      />
        ...
        <div className="container">
          <nav className="flex mt-2 mb-6 breadcrumb" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-1 md:space-x-3">
              ...
              <li
                aria-current="page"
                className="text-purpledark duration-500 hover: duration-500  text-sm hover:text-purplelight"
              >
                <div className="flex items-center">
                  <svg
                    className="w-6 h-6 text-gray-400"
                  ...
                  <span className="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">
                    {
                      data?.attributes?.Categoria_del_proyecto?.data?.attributes
                        ?.Nombre_de_Categoria
                    }
                  </span>
                </div>
          ...
          {arrIdProjects.length > 0 && (
            <div>
              <div className="bg-white p-2 sm:p-3 cursor-pointer duration-500 hover:bg-[#E6E6E6] absolute">
                <Link
                  href={`/proyectos/${query.query.categoriaproyectos}/detalle/${idNoticeBefore}`}
                >
              ...
              <div className="bg-white p-2 sm:p-3 cursor-pointer duration-500 hover:bg-[#E6E6E6] absolute right-0">
                <Link
                  href={`/proyectos/${query.query.categoriaproyectos}/detalle/${idNoticeAfter}`}
                >
          ...
          {query?.query?.categoriaproyectos?.includes("vivienda") ? (
            <>
              <div className="my-2 sm:my-16 min-height-project-vivienda">
                <h2 className="font-bold text-center text-3xl text-purpledark mb-5">
                  {data?.attributes?.Titulo}
                </h2>
                  ...
                  >
                    {data?.attributes?.Imagenes?.data?.map((image, index) => (
                      <SwiperSlide key={index}>
                        <img
                          className="w-full projects carrousel-gallery-right"
                          alt="image"
              ...
              <div className="my-2 sm:my-16 min-height-project responsive">
                <div className="topSpacerDiv"></div>
                  ...
                  >
                    {data?.attributes?.Imagenes?.data?.map((image, index) => (
                      <SwiperSlide key={index} className=" float-left p-3">
                        <img
                          className="w-full projects carrousel-gallery"
                          alt="image"
                          src={`${API_URL}${image.attributes.url}`}
                        />
                      </SwiperSlide>
                ...
                <div className="mx-4 sm:mx-0">
                  <h2 className="font-bold text-3xl text-purpledark mb-5">
                    {data?.attributes?.Titulo}
                  </h2>
                  <div className="content-project">
                    {HTMLReactParser(
            ...
            <div className="my-2 sm:my-16 min-height-project">
              <h2 className="font-bold text-3xl text-center text-purpledark mb-5">
                {data?.attributes?.Titulo}
              </h2>
              ...
              <div>
                ...
                >
                  {data?.attributes?.Imagenes?.data?.map((image, index) => (
                    <SwiperSlide key={index}>
                      <img
                        className="w-full projects carrousel-gallery cursor "
                        alt="image"
                        src={`${API_URL}${image.attributes.url}`}
                      />
                    </SwiperSlide>
